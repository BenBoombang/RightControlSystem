                        ...
                        <div class="pull-left">
                            <div class="form-group form-inline">
                                <div class="btn-group">
                                    ...
                                    <button type="button" class="btn btn-default" onclick="userOpenStatus()" title="开启">
                                        <i class="fa fa-check"></i> 开启
                                    </button>
                                    <button type="button" class="btn btn-default" onclick="userCloseStatus()" title="屏蔽">
                                        <i class="fa fa-ban"></i> 屏蔽
                                    </button>
                                    <button type="button" class="btn btn-default" onclick="window.location.reload();" title="刷新">
                                        <i class="fa fa-refresh"></i> 刷新
                                    </button>
                                    <button type="button" class="btn btn-default" onclick="userExportToExcel()" title="刷新">
                        ...
                        <div class="box-tools pull-right col-md-2">
                            <div class="has-feedback input-group">
									<span class="input-group-btn">
										<button class="btn btn-primary btn-sm" type="button" onclick="userPageQuery(1)">搜索</button>
									</span>
                                <input type="text" class="form-control input-sm"
                                       placeholder="用户名" name="queryText" id="queryText" />
                            </div>
                        </div>
                        <!--工具栏/-->
...
<script>
    ...
    // 批量导出
    function userExportToExcel() {
        // 首先获取筛选框中的条件
        var qryText = $("#queryText").val();
        window.location.href="${pageContext.request.contextPath}/user/export?queryText=" + qryText;
    }


    // 批量启用
    function userOpenStatus() {
        // 首先判断你有没有选中某行数据
        ...
        if (openBoxs.length == 0) {
            ...
            layer.confirm("是否要开启所选用户",  {icon: 3, title:'提示'}, function(cindex){
                layer.close(cindex);

                $.ajax({
                    type : "post",
                    url : "${pageContext.request.contextPath}/user/opens",
                    ...
                    success : function (result) {
                        if (result.succ) {
                            ...
                            layer.msg("开启用户失败", {time:2000, icon:5, shift:6}, function () {
                                // 回调方法
                            });
                        }
                    }
    ...
    function userCloseStatus() {
        ...
        if (closeBoxs.length == 0) {
            ...
            layer.confirm("是否要屏蔽所选用户",  {icon: 3, title:'提示'}, function(cindex){
                layer.close(cindex);

                $.ajax({
                    type : "post",
                    url : "${pageContext.request.contextPath}/user/closes",
                    data : $("#userForm").serialize(),
                    success : function (result) {
                        if (result.succ) {
                            ...
                            layer.msg("屏蔽用户失败", {time:2000, icon:5, shift:6}, function () {
                                // 回调方法
                            });
                        }
                    }
    ...
    function queryUserByIdAndAllRole(id) {
        window.location.href="${pageContext.request.contextPath}/user/queryUserByIdAndAllRole?id=" + id;
    }

    function userChangePageSize() {
        //获取下拉框的值
    ...
    function userPageQuery(pageno) {
        ...
        $.ajax({
            ...
            data : ajaxData,
            beforeSend : function () {
                userLoading = layer.msg('数据加载中',{icon:16});
            },
            success : function (result) {
                layer.close(userLoading);
                ...
                if (result.succ) {
                    ...
                    $.each(pageInfo.list,function (i,user) {
                        userContent += '<tr>';
                        userContent += '<td><input name="ids" type="checkbox" value="'+ user.id +'"></td>';
                        userContent += '<td>' + user.id + '</td>';
                        userContent += '<td>' + user.username + '</td>';
                        userContent += '<td>' + user.email + '</td>';
                        userContent += '<td>' + user.phoneNum + '</td>';
                        userContent += '<td>' + user.statusStr + '</td>';
                    ...
                    footContent += '总共 <strong> ' +pageInfo.pages + ' </strong> 页，共 <strong> ' + pageInfo.total + ' </strong> 条数据。每页 <select class="form-control" id="selectPageSize" onchange="userChangePageSize()">';
